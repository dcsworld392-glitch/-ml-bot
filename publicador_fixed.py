...
{{
  ...
  "score_estimado": 82
            ...
            if titulo_nuevo and titulo_nuevo != titulo_actual:
                ...
                mejoras_aplicadas.append("titulo")
            ...
            if atributos_nuevos:
                ...
                if attrs_map != {a.get("id"): a for a in attrs_actuales}:
                    updates["attributes"] = list(attrs_map.values())
                    mejoras_aplicadas.append("atributos")

            if updates:
                try:
                    resp_put = self.ml.put(f"/items/{item_id}", updates)
                    if isinstance(resp_put, dict) and resp_put.get("error"):
                        return {"ok": False, "item_id": item_id, "error": f"ML rechazo PUT: {resp_put.get('message','')}", "resp": str(resp_put)[:300]}
                except Exception as e_put:
                    return {"ok": False, "item_id": item_id, "error": f"Error PUT items: {str(e_put)}"}

            # 7. Actualizar descripción — SIEMPRE si Claude la generó
            desc_nueva = mejora.get("descripcion_nueva", "")
            if desc_nueva and len(desc_nueva) > 100:
                try:
                    if desc_actual:
                    ...
                    mejoras_aplicadas.append("descripcion")
                except Exception as e_desc:
                    pass  # descripcion no es bloqueante
